...
public class DosenActivity extends AppCompatActivity {
    ...
    private void handleIntent(Intent intent) {
        fakultas = (Fakultas) intent.getSerializableExtra("fakultas");
    ...
    private void loadDataDosen() {
        ...
        Call<List<Dosen>> call = services.getDosenByProdi(fakultas.getKODEPRODI());
        call.enqueue(new Callback<List<Dosen>>() {
            @Override
            public void onResponse(Call<List<Dosen>> call, final Response<List<Dosen>> response) {
                pd.dismiss();
                if(response.isSuccessful()){
                    DosenAdapter adapter = new DosenAdapter(DosenActivity.this, response.body());
                    rv.setAdapter(adapter);
                    adapter.setClickListener(new DosenAdapter.ClickListener() {
                        @Override
                        public void onClick(View v, int position) {
                            Intent intent = new Intent(DosenActivity.this, DosenMapsActivity.class);
